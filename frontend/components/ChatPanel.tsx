...
const ChatPanel = React.forwardRef<any, { user?: any }>((props, ref) => {
	...
	const generateChatTitle = (firstMessage: string): string => {
		...
		const relevantWords = words.filter(
			(word) => keywords.includes(word) || word.length > 4
		);
		...
		if (relevantWords.length >= 2) {
			...
			return (
				relevantWords[0].charAt(0).toUpperCase() +
				relevantWords[0].slice(1) +
				" Chat"
			);
		} else {
			return words
	...
	const handleSend = async (e: React.FormEvent) => {
		...
		try {
			await conversationService.sendMessage(userInput, conversationId);
			...
			const pollForResponse = async () => {
				try {
					...
					if (
						agentMessage &&
						agentMessage.parts &&
						agentMessage.parts.length > 0
					) {
						const responseText = agentMessage.parts[0].text;
						setMessages((prev) => {
							if (prev.find(m => m.id === agentMessage.messageId)) {
								return prev;
							}
	...
	return (
		<div
			className={`h-full flex flex-col bg-white dark:bg-gray-900 transition-all duration-500 ${
